...
chipset=$(grep -i 'hardware' /proc/cpuinfo 2>/dev/null | uniq | cut -d ':' -f2 | sed 's/^[ \t]*//')
[ -z "$chipset" ] && chipset="$(getprop ro.board.platform) $(getprop ro.hardware) $(getprop ro.mediatek.platform)"
chipset_lower=$(echo "$chipset" | tr '[:upper:]' '[:lower:]')

case "$chipset_lower" in
*mt* | *mediatek* | *helio* | *dimensity*)
	soc="MediaTek"
	echo "- MediaTek SoC confirmed: $chipset"
	echo "- Applying optimizations for $soc"
	setprop persist.sys.azenith.soctype 1
	;;
*)
	abort "! Error: AZenith is exclusively optimized for MediaTek devices only! Detected: $chipset"
	;;
esac
